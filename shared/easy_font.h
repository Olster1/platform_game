...
Font initFont_(char *fileName, char firstChar, char numOfChars, int fontHeight)
{
    Font result = {}; 
    ...
    result.cdata = (stbtt_bakedchar *)calloc(numOfChars*sizeof(stbtt_bakedchar), 1);
    //
    ...
    #else 
        result.handle = openGlLoadTexture(FONT_SIZE, FONT_SIZE, bitmapTexture);
    #endif
    return result;
}
...
Font initFont(char *fileName, int fontHeight) {
    Font result = initFont_(fileName, 32, 96, fontHeight); // ASCII 32..126 is 95 glyphs
    return result;
}
...
Rect2f my_stbtt_print_(Font *font, float x, float y, float bufferWidth, float bufferHeight, char *text_, Rect2f margin, V4 color, float size, CursorInfo *cursorInfo, bool display) {
    ...
    while (*text) {
        ...
        if (((int)(*text) >= 32 && (int)(*text) < 128)) {
            stbtt_aligned_quad q  = {};
            stbtt_GetBakedQuad(font->cdata, FONT_SIZE, FONT_SIZE, *text-32, &x,&y,&q, 1);
...
Rect2f outputTextWithLength(Font *font, float x, float y, float bufferWidth, float bufferHeight, char *allText, int textLength, Rect2f margin, V4 color, float size, bool display) {
